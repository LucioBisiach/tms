...
class ExpiryDocumentAttachment(models.Model):
    ...
    doc_attach_expiry_rel = fields.Many2many('vencimientos.tms', 'vencimientos_doc_attachment_id', 'attach_id3', 'doc_id',
                                    string="Adjunto", invisible=1)


class TypeExpiryDocumentsTms(models.Model):
    _name = 'type.vencimientos.tms'
    ...
    tipe = fields.Selection([
        ...
        ('employee','Empleado'),
        ('fleet','Flota'),
        ('otros', 'Otros')], string="Tipo", required=True)

    days_notification = fields.Integer(string="Días de Aviso") 

class ExpiryDocumentsTms(models.Model):
    _name = 'vencimientos.tms'
    _description = ''
    
    name = fields.Many2one('type.vencimientos.tms', string="Descripción")
    ...
    tipe = fields.Char(string="Tipo", invisible=True, readonly=True)
